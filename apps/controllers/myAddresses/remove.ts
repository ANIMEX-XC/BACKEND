...
export const removeMyAddress = async (req: any, res: Response): Promise<any> => {
  const requestQuery = req.query as MyAddressesAttributes
  ...
  try {
    const result = await MyAddressesModel.findOne({
      where: {
        deleted: { [Op.eq]: 0 },
        myAddressId: { [Op.eq]: requestQuery.myAddressId }
      }
    })
